...
interface BookPageProps {
  ...
  /** All highlights that belong to this page */
...
export const BookPage = React.forwardRef<HTMLDivElement, BookPageProps>(
  ({
    chapter, page, pageIndex, totalPages,
    isPortrait = true, onComplete,
    watermarkIdentity, isHighlightMode = false,
    pageHighlights = [], pulsingHighlightId,
  }, ref) => {
    ...
    return (
      <div
        ...
        style={{
          ...
          position:       'relative',
          // Allow text selection in highlight mode; block it otherwise
      ...
      >
        ...
        <div
          style={{
            position: 'absolute', top: '1px', left: 0, right: 0, bottom: '24px',
...
function ClosingContent({ onComplete, isPortrait }: { onComplete?: () => void; isPortrait: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      <div style={{ width: '20px', height: '1px', background: C.inkHair, marginBottom: '24px' }} />
      <span style={{ display: 'block', fontSize: '9px', fontWeight: 700, letterSpacing: '0.2em', color: C.inkFaint, marginBottom: '26px' }}>
        FIM DO CAPÍTULO
      </span>
      <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', fontSize: isPortrait ? '18px' : '16px', color: C.inkMid, lineHeight: 1.78, marginBottom: '32px', maxWidth: '82%' }}>
      ...
      <button
        ...
        onClick={onComplete}
        style={{ background: 'transparent', border: `1px solid ${C.inkHair}`, borderRadius: '100px', padding: '10px 24px', color: C.inkMid, fontSize: '12px', fontWeight: 500, letterSpacing: '0.04em', cursor: 'pointer', transition: 'border-color 0.25s, color 0.25s' }}
        onMouseEnter={e => { e.currentTarget.style.borderColor = C.purpleFaint; e.currentTarget.style.color = '#251508'; }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = C.inkHair; e.currentTarget.style.color = C.inkMid; }}
      >
        Concluir capítulo
      </button>
    </div>
...
function PageBlocks({ blocks, typo, isPortrait, isHighlightMode, pageHighlights, pulsingHighlightId }: PageBlocksProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      {blocks.map((block, idx) => (
        <PageBlock
          ...
          isPortrait={isPortrait}
          isHighlightMode={isHighlightMode}
          blockHighlights={pageHighlights.filter(h => h.blockIdx === idx)}
          pulsingHighlightId={pulsingHighlightId}
        />
      ))}
    </div>
...
function PageBlock({ block, blockIdx, typo, isPortrait, isHighlightMode, blockHighlights, pulsingHighlightId }: PageBlockProps) {
  switch (block.type) {
    // ── Text ─────────────────────────────────────────────────────────────────
    case 'text':
      return (
    ...
    case 'reflection':
      return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: isPortrait ? '24px 20px' : '18px 16px', background: C.inkHair, borderRadius: '10px', border: `1px solid ${C.inkGhost}`, position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', fontSize: isPortrait ? '20px' : '18px', color: '#251508', lineHeight: 1.6, textAlign: 'center', margin: '0 0 16px 0' }}>"{block.question}"</p>
    ...
    case 'practice':
      return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: isPortrait ? '24px 20px' : '18px 16px', border: `1px solid ${C.purpleFaint}`, borderRadius: '10px', background: C.purpleHair }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: isPortrait ? '20px' : '18px', color: '#251508', lineHeight: 1.65, textAlign: 'center', margin: 0 }}>{block.instruction}</p>
        </div>
      );
    ...
    case 'audio':
      return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: isPortrait ? '18px 20px' : '14px 16px', background: C.inkHair, borderRadius: '12px', border: `1px solid ${C.inkGhost}`, cursor: 'pointer' }}>
    ...
    case 'video':
      return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: 0 }}>
          <div style={{ width: '100%', aspectRatio: '16/9', borderRadius: '10px', background: C.inkHair, border: `1px solid ${C.inkGhost}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
