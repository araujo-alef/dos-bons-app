...
export function AppHeader() {
  ...
  return (
    <header className="flex items-center justify-between pt-8 pb-0 px-4">
      {/* Spacer so the tagline stays centred when the avatar is visible */}
      <div className="w-8" />

      <Link href="/" className="no-underline text-center flex-1">
        <p
          style={{
      ...
      {user ? (
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-10 w-52 bg-[#111] border border-white/10 rounded-xl shadow-xl shadow-black/50 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
        ...
        /* Keep the right side balanced when not logged in */
        <div className="w-8" />
      )}
    </header>
  );
